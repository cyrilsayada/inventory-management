...
3. **Always document non-obvious logic changes with inline comments.** If a future reader (human or model) has to step through the code to figure out _why_, the comment is missing. State the intent and the constraint that drove it — not the mechanics, the code already shows those.
...
5. **When a feature works, ship it — don't leave it loose on `main`.** As soon as a discrete piece of work passes its smoke test, the default is: create a feature branch, commit, push, open a PR. Don't wait to be asked. Specifically:
   ...
   3. **Commit with a structured message.** Subject line: imperative, under 70 chars (`Add Restocking tab and Sia redesign`). Body: bullet sections — `Feature work:`, `UI:`, `Docs:`, `Config:`, etc. — explaining the _why_, not the file list. Always end with the `Co-Authored-By: Claude …` trailer.
   ...
   5. **Open the PR — base = the FORK, never upstream.** First try `mcp__github__create_pull_request` with `owner: "cyrilsayada"`, `repo: "inventory-management"`, `base: "main"`, `head: "<branch>"`. **NEVER** call it with `owner: "beck-source"`. If GitHub MCP auth fails, fall back to a browser URL — but **use the `compare` route, not `pull/new`**, because `pull/new/<branch>` on a fork defaults base to the UPSTREAM parent (this has bitten us — see `LEARNINGS.md` 2026-05-21 entry on GitHub URL defaults). Correct URL: `https://github.com/cyrilsayada/inventory-management/compare/main...<branch>?expand=1`. Open via `Start-Process`. **Always sanity-check the rendered "base: ... ← compare: ..." selector before telling the user to click Merge.**
